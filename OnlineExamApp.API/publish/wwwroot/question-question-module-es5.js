...
var QuestionComponent = /** @class */ (function () {
    function QuestionComponent(cookie, route, router, questionService, authService) {
        this.cookie = cookie;
        this.route = route;
        this.router = router;
    ...
    QuestionComponent.prototype.ngOnInit = function () {
        ...
        this.route.data.subscribe(function (data) {
            _this.authService.canUpdateTrials(data.question.trials);
            localStorage.setItem('trials', data.question.trials);
            _this.getQuestion(data.question.questionsCollections);
        });
        this.questionService.seconds = 1000;
        this.setTimer();
        this.radioSelected = 'Lamba1';
    };
    QuestionComponent.prototype.setTimer = function () {
        var _this = this;
        this.questionService.timer = setInterval(function () {
            _this.questionService.seconds--;
            if (_this.questionService.seconds === 0) {
                _this.submitQuestion();
            }
        }, 1000); // the function will be called after a second
    };
    QuestionComponent.prototype.selectedOption = function (value, QId) {
        console.log(value, QId);
        var question = this.allQuestion.find(function (x) { return x.questionId === QId; });
        question.selectedAnswer = value;
        try {
            var values = question.options.find(function (x) { return x.checked === true; });
            values.checked = false;
            var newValues = question.options.find(function (x) { return x.optionId === value; });
    ...
    QuestionComponent.prototype.getSelecteditem = function (QId, optionId) {
        try {
            this.currentQuestion = this.allQuestion.find(function (x) { return x.questionNumber === QId; });
            this.radioSel = this.currentQuestion.options.find(function (Item) { return Item.checked === true; });
            this.radioSel.checked = false;
            this.updateRadioSel = this.currentQuestion.options.find(function (x) { return x.optionId === optionId; });
    ...
    QuestionComponent.prototype.goTo = function (QId) {
        try {
            this.currentQuestion = this.allQuestion.find(function (x) { return x.questionNumber === QId; });
            this.updateRadioSel = this.currentQuestion.options.find(function (x) { return x.checked === true; });
            this.radioSelected = this.updateRadioSel.optionName;
        }
        catch (e) {
        }
    };
    QuestionComponent.prototype.getQuestion = function (question) {
    ...
    QuestionComponent.prototype.submitQuestion = function () {
        ...
        for (var i = 0; i < this.allQuestion.length; i++) {
            var joko = this.allQuestion[i].questionId;
        ...
        this.questionService.Submit(this.answerArray).subscribe(function (data) {
        }, function (error) {
        }, function () {
...
var QuestionResolver = /** @class */ (function () {
    function QuestionResolver(questionService, router) {
        this.questionService = questionService;
        this.router = router;
    }
    QuestionResolver.prototype.resolve = function (route) {
        var _this = this;
        return this.questionService.GetQuestion(+route.params['id']).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["catchError"])(function (error) {
            _this.router.navigate(['/members']);
            return Object(rxjs__WEBPACK_IMPORTED_MODULE_4__["of"])(null);
        }));
    };
    QuestionResolver.ctorParameters = function () { return [
...
//# sourceMappingURL=question-question-module-es5.js.map