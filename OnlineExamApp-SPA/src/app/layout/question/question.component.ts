...
this.questionService.seconds--;
 ...
 goTo(QId: any) {
   ...
   try {
    const value = this.currentQuestion.options.find(x => x.checked === true);

     this.materialExampleRadios = 4;
      this.currentQuestion.selectedAnswer = value.optionId;

   } catch (e) {
    this.materialExampleRadios = 0;
   }
